...
public class FindAllAnagrams {
	public static void main(String[] args) {
		...
		//System.out.println(sol.isAnagram("ab", "ac"));
		...
		*/
		...
		public List<Integer> findAnagrams5(String s, String p) {
	        ...
	        while(right < ss.length) {
	            ...
	            while (matchSize == 0) { // while loop to skip chars not in the pattern p
	                if (right - left  == pp.length) {
	                    ret.add(left);
	                }
                    
                    // restore it by 1, which was reduced by right pointer before
	                counts[ss[left]]++;
	                if (counts[ss[left]] > 0) {
	                    matchSize++;
	                }
		...
		public List<Integer> findAnagrams4(String s, String p) {
			...
			while (end < str.length) {
				if (map.containsKey(str[end])) {
					int temp = map.get(str[end]) - 1;
					map.put(str[end], temp);
					if (temp >= 0)
						--count;
				}
				...
				if (end - start == pattern.length) {
					if (map.containsKey(str[start])) {
						int temp = map.get(str[start]) + 1;
						map.put(str[start], temp);
						if (temp > 0) 
							++count;
					}
					++start;
	...
	public List<Integer> findAnagrams(String s, String p) {
        ...
        for (int i = 0; i < ss.length - pp.length + 1; i++) {
            if (isAnagram(ss, i, pp.length, counts))
                list.add(i);
        }
    ...
    boolean isAnagram(char[] s, int start, int len, HashMap<Character, Integer> counts) {
        HashMap<Character, Integer> map = new HashMap<>();
		...
		public List<Integer> findAnagrams(String s, String p) {
            ...
            char[] pp = p.toCharArray(); 
	           ...
	           while (right < ss.length){
	               // if this char exists in p, then reduce matchSize
	               if (map[ss[right]] > 0)
	                   matchSize--;
	                map[ss[right]]--;
	               ...
	               if (right - left == pp.length-1){
	                   ...
	                   // restore it by 1, which was duducted before by the right pointer
	                   map[ss[left]]++;
	                   
	                   if (map[ss[left]] > 0)
	                       matchSize++;
	                   
	                   left++; // move left pointer to start new search
	               }
	              // move right
	              right++;
	           }
